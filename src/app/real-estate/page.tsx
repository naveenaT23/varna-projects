    ...
    <main className="flex min-h-screen flex-col items-center justify-start pt-32 md:pt-48 px-6 md:px-12 pb-24">
          ...
          {realEstateData.map((service, i) => (
            ...
            >
              <Link href={`/real-estate/${service.slug}`} className="group flex flex-col md:flex-row items-stretch bg-transparent border border-white/10 rounded-sm overflow-hidden hover:border-gold-base/30 transition-colors duration-700 cursor-pointer w-full h-full">
                <div className="w-full md:w-2/5 h-64 md:h-[400px] bg-[#e5e5e5] relative overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 z-10" />
                  <img src={service.image} alt={service.title} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000 grayscale group-hover:grayscale-0" />
                </div>
      ...
      <section className="w-full max-w-6xl mx-auto py-24 border-t border-white/10 mt-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {[
            { name: "3BHK Premium Suite", size: "2,400 Sq.Ft.", type: "Residential", image: "/images/layout.png" },
            { name: "4BHK Luxury Villa", size: "4,500 Sq.Ft.", type: "Villa", image: "/images/lumina_estates.png" },
          ].map((layout, i) => (
            <motion.div
              key={layout.name}
              initial={{ opacity: 0, y: 30 }}
            ...
            >
              <div className="w-full h-64 mb-8 overflow-hidden rounded-xl bg-midnight border border-white/5">
                 <img src={layout.image} alt={layout.name} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 group-hover:scale-105 transition-all duration-700 mix-blend-luminosity group-hover:mix-blend-normal" />
              </div>
